...
## Create a router
### Request
When creating a router you can either specify the router `ref` ID for example `MY_QUEUE` or leave blank and it will be automatically generated.
```
curl -X PUT http://localhost:8080/comms-router-web/api/routers/MY_QUEUE
```
...
```
The URL of the new created resource is also provided in the header LOCATION of the response `Location: http://localhost:8080/comms-router-web/api/routers/HaOYogXa8qgX9NlRHJi9Y2`


## Create a queue
```
curl -X PUT http://localhost:8080/comms-router-web/api/routers/my-router/queues/MY_QUEUE 
  -H "Content-Type:application/json" 
  -d "{\"predicate\":\"HAS(#{language},'es')\"}}"
```
